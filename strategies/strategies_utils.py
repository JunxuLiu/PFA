...
class _Model:
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        self.model = copy.deepcopy(model).to(self._device)
    ...
    def _make_private(self, privacy_engine):
        ...
        if not isinstance(acct.sample_rate, float):
            self.model, self._optimizer, self._train_dl, acct = privacy_engine.make_private_with_rpdpfl(
                module=self.model,
            ...
            self.model, self._optimizer, self._train_dl, acct = privacy_engine.make_private_with_dpfl(
                module=self.model,
                optimizer=self._optimizer,
                data_loader=self._train_dl,
                accountant=acct
