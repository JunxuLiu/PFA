...
class Client(object):
    ...
    def set_projection(self, Vk=None, mean=None, project=False):
    ...
    def local_update(self, sess, model, global_steps):
        ...
        update = [(model[Vname_to_FeedPname(var)] - sess.run(var)) for var in tf.trainable_variables()]

        if (self.Vk is not None) and self.project:
            update_1d = [u.flatten() for u in update]
            print('self.mean: {}'.format(len(self.mean)))
            update = [ np.dot(self.Vk[i].T, update_1d[i]-self.mean[i]) for i in range(len(update_1d)) ]
    
        print('update[0].shape: {}'.format(update[0].shape))
        # update the budget accountant
        accum_bgts = self.ba.update(self.loc_steps) if self.ba is not None else None

        return update, accum_bgts
